...
class LowPassDiscreteFilter:
    ...
    def __init__(self, wc, Ts, Tsim = Ts, order = 1, DiscType = 'tustin'):
        self.Tc = 1/wc
        self.Ts = Ts
        self.y   = 0
        self.ym1 = 0
    ...
    def step(self,u):
        ...
        if self.DiscType == 'tustin':
            self.y = 1 / (2 * self.Tc + self.Ts) * (self.Ts * (self.u + self.um1) - (self.Ts-2*self.Tc) * self.ym1)
        
        elif self.DiscType == 'euler_fwd':
            self.y = 1 / (self.Tc + self.Ts) * (self.Ts * self.um1 + self.Tc * self.ym1)

        elif self.DiscType == 'euler_back':
            self.y = 1 / (self.Tc + self.Ts) * (self.Ts * self.u + self.Tc * self.ym1)
...
Engine4 = LowPassDiscreteFilter(wc = wc, Ts=ts, DiscType = 'tustin')
...
Engine = LowPassDiscreteFilter(wc = wc, Ts=t_filt, DiscType = 'euler_back')
...
for i in range(1,len(t_vec)):
    ...
    for j in range(sf):
        y_vec[i] = Engine.step(u_vec[i-1]*j/(sf-1) + u_vec[i]*(sf-j-1)/(sf-1))
        # y_vec[i] = Engine.step(u_vec[i])
        # y_vec[i] = Engine.step(u_vec[i-1])
