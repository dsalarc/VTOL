...
TrimVec['ContAlloc_Thr'] = TrimVec['PitchThrottle_to_qdot'] / \
    (TrimVec['PitchThrottle_to_qdot'] + TrimVec['Elevator_to_qdot'])
...
for nv_trim in range(len(TrimVec['VX_mps'])):
    ...
    TrimVec['TrimTilt_deg'][nv_trim] = TrimTilt_deg
    ...
    for nt in range(len(TrimVec['AX_mps2'])):
        ...
        if np.abs(Trimmed[nv_trim, nt_trim] - 1) > 0.1:
            ...
            t = time.time()
            OptGains = opt.minimize(GeneralPitchFunction, x0, args=(
...
File2Save = {'GainsVec': GainsVec, 'TrimVec': TrimVec, 'CostVec': CostVec, 'SaveTrim': SaveTrim}
