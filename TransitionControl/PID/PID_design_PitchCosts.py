...
def CalculateIndividualCosts(ClosedLoops):
    ...
    T, yout = ct.step_response(ClosedLoops['ThetaOpen'] , T=5, input = ClosedLoops['ThetaOpen'].input_labels.index('Q_ref_degps'))
    max_q = np.max(yout[ClosedLoops['ThetaOpen'].output_labels.index('Q_degps')][0])
    ...
    T, yout = ct.step_response(ClosedLoops['Complete'] , T=5, input = ClosedLoops['Complete'].input_labels.index('Theta_ref_deg'))
    max_T = np.max(yout[ClosedLoops['Complete'].output_labels.index('Theta_deg')][0])
    ...
    T, yout = ct.step_response(ClosedLoops['Complete'] , T=10, input = ClosedLoops['Complete'].input_labels.index('Theta_ref_deg'))
    over_riseLim = T[yout[ClosedLoops['Complete'].output_labels.index('Theta_deg')][0] > 0.8]
    if len(over_riseLim) == 0:
        theta_rt = 10
    else:
        theta_rt = T[yout[ClosedLoops['Complete'].output_labels.index('Theta_deg')][0] > 0.8][0]
    ...
    if np.isnan(dcgain):
        T,yout = ct.step_response(ClosedLoops['OnlyQ'],10)
        dcgain = yout[-1]
    ...
    dcgain = ct.dcgain(ClosedLoops['OnlyQ'])
    if np.isnan(dcgain):
        T,yout = ct.step_response(ClosedLoops['OnlyQ'],10)
        dcgain = yout[-1]
    ...
    aux = ct.damp(ClosedLoops['OnlyQ'] , doprint=False)
