...
reset_INPUT_VEC['WIND_TowerX_mps'] = np.array([[0, 20, 40] , [WindX, WindX, WindX]])
reset_INPUT_VEC['WIND_TowerY_mps'] = np.array([[0, 20, 40] , [0, 0, 0]])
reset_INPUT_VEC['WIND_TurbON']     = np.array([[0, 40] , [TurbON, TurbON]])
...
Ref['VZ_mps'] = np.array([[0 , SimTime ],
                          [0 ,  0      ]])
Ref['VX_mps'] = np.array([[0 , 5 , 5+TransTime, SimTime ],
                          [0 , 0 , 60 , 60    ]])
...
Ref['ControlMerge'] = np.array([[0 , 40.0 , 60.0, 100.0 ],
                                [0 ,    0 ,    1,     1 ]])
...
for step in range(n_steps):
    ...
    if ((not start_FWControl) and (ControlMerge > 0.0001)):
        ...
        FWController = GenFWController(Gains['FwdVert'], Last_t = 0.0, Throttle0 = 0.0 , Theta0 = 0.0)
...
fig.savefig('/home/dsalarc/Documents/DOUTORADO/Environments/VTOL/TransitionControl/NonLinearResponse_FinalSimulation_Wind' + '{:02d}'.format(WindX) + '_Turb' + '{:01d}'.format(TurbON) + '_' + '{:01d}'.format(TransTime) + 's.pdf', bbox_inches='tight')
...
if False:
    ...
    fig.savefig('/home/dsalarc/Documents/DOUTORADO/Environments/VTOL/TransitionControl/LinearResponse_FinalSimulation.pdf', bbox_inches='tight')
    print('Fig Saved')
